"""Preview tweaks: translucent panel restored, 1px separator, progress bar moved up."""
import os
...
F_STATUS=font("DejaVuSans.ttf",30); F_BTN=font("DejaVuSans-Bold.ttf",34); F_GHOST=font("DejaVuSans.ttf",30)
...
d=ImageDraw.Draw(img,"RGBA")
w1=tw(d,"NAZWA SERWERA",F_TITLE,12); spaced(d,(W-w1)/2,60,"NAZWA SERWERA",F_TITLE,GOLD,12,glow=(0,0,0))
w2=tw(d,"LAUNCHER",F_SUB,10); spaced(d,(W-w2)/2,150,"LAUNCHER",F_SUB,DIM,10)

# ── bottom panel: translucent (0.70), thin 1px(=2px@2x) gold separator ──
ptop=H-250
d.rectangle([0,ptop,W,H],fill=(20,20,20,179))          # 0.70 opacity (was 0.95)
d.rectangle([0,ptop,W,ptop+2],fill=GOLD+(255,))         # 1px logical separator (2px @2x)

# status + progress moved UP (progress sits just below status, near the separator)
...
# square gold frame (no rounding)
d.rectangle([0,0,W-1,H-1],outline=GOLD,width=4)
img.save(os.path.join(OUT_DIR,"tweak.png")); img.save(os.path.join(ROOT,"tweak.png"))
print("done")
